...
| PUT | `/api/token/` | 使用 User 权限修改已有 API 密钥的远端分组 |
...
编辑已导入的二狗子 Profile 时可选择远端分组。实现先读取远端实时令牌详情，再用该快照构造 `PUT /api/token/` 请求，仅替换 `group`，避免 New API 更新接口将缺省字段覆盖为零值，或用本地过期的额度、有效期、模型/IP 限制覆盖官网设置。远端成功后才同步本地令牌分组和 Profile 编辑内容；传输、超时、解析或 5xx 错误同样按结果未知处理，不自动重试且不修改本地分组。
