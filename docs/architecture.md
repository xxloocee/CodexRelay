...
二狗子管理请求使用 `/api/user/self`、`/api/user/self/groups`、分页 `/api/token/`；绑定和手动同步通过 `POST /api/token/{id}/key` 获取完整密钥，定时同步仅为新增或本地缺失密钥的令牌调用该接口。主页创建密钥使用相同访问令牌调用 `POST /api/token/`，当前只提交名称、远端分组、永不过期和不限额度参数；由于创建响应不返回令牌 ID，后端在创建前后读取目录，只有唯一同名新增项才能继续读取并保存完整 key。创建请求出现传输、超时或服务端错误时按结果未知处理，要求用户先手动同步，不自动认领目录候选。完整密钥不进入 `DesktopState` 或前端参数。实际代理连通性仍通过令牌自身访问 `/v1/models` 探测。代理路由覆盖所有已知类别；生图和其他类别只记录请求次数，不旁路解析 Token 用量。
